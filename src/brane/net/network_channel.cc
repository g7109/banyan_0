...
network_message_queue::network_message_queue(seastar::reactor* from, seastar::reactor* to)
    : _pending(to)
    , _completed(from) {}
...
void network_channel::create_qs(const std::vector<seastar::reactor*>& reactors) {
    ...
    for (unsigned i = 0; i < nr_cpu; i++) {
        network_channel::_qs[i] = reinterpret_cast<network_message_queue*>(operator new[] (sizeof(network_message_queue) * nr_cpu));
        for (unsigned j = 0; j < nr_cpu; ++j) {
            new (&network_channel::_qs[i][j]) network_message_queue(reactors[j], reactors[i]);
        }
    }
}
