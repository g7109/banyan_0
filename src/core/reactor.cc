...
#include <brane/core/coordinator_pollfn.hh>
...
seastar::logger reactor_logger("reactor");

template <typename... Args>
void actor_print(const char* fmt, Args&&... args) {
#ifdef REACTOR_DEBUG
    reactor_logger.info(fmt, std::forward<Args>(args)...);
#endif
}
...
reactor::reactor(unsigned id, reactor_backend_selector rbs, reactor_config cfg)
    ...
    try {
        ...
        fmt::print("Runtime error in reactor {}: {}\n", id, e.what());
    }
#ifdef HAVE_OSV
    _timer_thread.start();
#else
...
void reactor::run_tasks(task_queue& tq) {
    ...
    *internal::current_scheduling_group_ptr() = scheduling_group(tq._id);
    ...
    while (!tasks.empty()) {
        ...
        brane::actor_clock = std::chrono::steady_clock::now(); // DAIL
        brane::set_actor_quota(brane::default_actor_quota); // DAIL
        tsk->run_and_dispose();
        tsk.release();
...
// [DAIL]
#ifdef BRANE_GPU_RESOURCE_POOL
class reactor::gpu_resource_pollfn final : public reactor::pollfn {
    reactor& _r;
public:
    gpu_resource_pollfn(reactor& r) : _r(r) {}
    virtual bool poll() final override {
        return brane::gpu_resource_pool::poll_queues();
    }

    virtual bool pure_poll() override final {
        return brane::gpu_resource_pool::pure_poll_queues();
...
public:
    ...
    virtual bool poll() final override {
        return (smp::poll_queues() |
                ...
                ::brane::local_channel::poll_queues() |
                ::brane::network_channel::poll_queues());
    }
    virtual bool pure_poll() final override {
        return (smp::pure_poll_queues() ||
                alien::smp::pure_poll_queues() ||
                ::brane::local_channel::pure_poll_queues() ||
                ::brane::network_channel::pure_poll_queues());
    }
    virtual bool try_enter_interrupt_mode() override {
...
// [DAIL] resource thread pool poolfn
class reactor::thread_resource_pollfn final : public reactor::pollfn {
    reactor& _r;
public:
    thread_resource_pollfn(reactor& r) : _r(r) {}
    virtual bool poll() final override {
        return brane::thread_resource_pool::poll_queues();
    }
    virtual bool pure_poll() final override {
        return brane::thread_resource_pool::pure_poll_queues();
    }

    virtual bool try_enter_interrupt_mode() override {
        brane::thread_resource_pool::enter_interrupt_mode();
        if (poll()) {
            // raced
            brane::thread_resource_pool::enter_interrupt_mode();
            return false;
    ...
    virtual void exit_interrupt_mode() override final {
        brane::thread_resource_pool::exit_interrupt_mode();
    }
};
...
int reactor::run() {
    ...
    // [DAIL]
    compat::optional<poller> thread_resource_poller = {};
    if(brane::thread_resource_pool::active()) {
        thread_resource_poller = poller(std::make_unique<reactor::thread_resource_pollfn>(*this));
    }

    #ifdef BRANE_GPU_RESOURCE_POOL
        poller gpu_resource_poller(std::make_unique<gpu_resource_pollfn>(*this)); 
    #endif

    compat::optional<poller> coord_poller = {};
    if (_id == 0) {
        coord_poller = poller(std::make_unique<brane::coordinator_pollfn>());
    }
    ...
    while (true) {
        ...
        if (_stopped) {
            ...
            if (_id == 0) {
                smp::join_all();
                // [DAIL]
                brane::thread_resource_pool::stop();
#ifdef BRANE_GPU_RESOURCE_POOL
                brane::gpu_resource_pool::stop();
#endif
            }            
            break;
        }
...
void init_default_smp_service_group() {
    ...
    // TODO[DAIL]: remove this line later.
    brane::init_default_actor_msg_service_group();
}
...
void schedule(std::unique_ptr<task> t) {
    // [DAIL]
    auto ctx = t->get_execution_context();
    ctx == nullptr ? engine().add_task(std::move(t)) : ctx->add_task(std::move(t));
}

void schedule_urgent(std::unique_ptr<task> t) {
    // [DAIL]
    auto ctx = t->get_execution_context();
    ctx == nullptr ? engine().add_urgent_task(std::move(t)) : ctx->add_urgent_task(std::move(t));
}
...
reactor::get_options_description(reactor_config cfg) {
    ...
    opts.add_options()
        ...
        ("max-thread-resource-count", bpo::value<uint32_t>()->default_value(32),
	  	  		"Max thread resource count.")
	  	("init-thread-resource-count", bpo::value<uint32_t>()->default_value(8),
	  	  		"Initial thread resource count.")
	  	("thread-resource-retry-interval-us", bpo::value<int64_t>()->default_value(500),
	  	  		"Thread resource re-acquire interval in us.")
#ifdef BRANE_GPU_RESOURCE_POOL
        ("cuda-stream-resource-number", bpo::value<uint32_t>()->default_value(4),
	  	  		"Cuda stream resource number.")
        ("max-gpu-task-number-per-stream", bpo::value<uint32_t>()->default_value(2),
...
void smp::start_all_queues()
{
    for (unsigned c = 0; c < count; c++) {
        if (c != engine().cpu_id()) {
	        ...
	        // [DAIL]
	        ::brane::local_channel::_qs[c][engine().cpu_id()].start(c);
            ::brane::network_channel::_qs[c][engine().cpu_id()].start(c);
        }
    }
...
void smp::cleanup_cpu() {
    ...
    // [DAIL]
    if (::brane::local_channel::_qs) {
        for (unsigned i = 0; i < smp::count; i++) {
            ::brane::local_channel::_qs[i][cpuid].stop();
        }
    }
    if (::brane::network_channel::_qs) {
        for (unsigned i = 0; i < smp::count; i++) {
            ::brane::network_channel::_qs[i][cpuid].stop();
        }
    }
    if (alien::smp::_qs) {
...
void smp::configure(boost::program_options::variables_map configuration, reactor_config reactor_cfg)
{
    ...
    // [DAIL]
    brane::allocate_actor_engine();
    brane::coordinator::get().launch_worker(&engine());
    ...
    // [DAIL]
    ::brane::local_channel::create_qs(_reactors);
    ::brane::network_channel::create_qs(_reactors);
    ::brane::thread_resource_pool::configure(configuration);
#ifdef BRANE_GPU_RESOURCE_POOL
    ::brane::gpu_resource_pool::configure(configuration);
#endif
