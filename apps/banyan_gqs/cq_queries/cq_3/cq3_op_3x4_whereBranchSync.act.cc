...
void cq3_op_3x4_whereBranchSync::set_owner(TrivialStruct<int64_t> &&vertex) {
    _owner = vertex.val;
    _owner_set = true;
    if (_finished) { notify_and_cancel(true); }
}

void cq3_op_3x4_whereBranchSync::set_successful() {
    if (_finished) { return; }
...
void cq3_op_3x4_whereBranchSync::receive_eos(PathEos &&eos) {
    if (_finished) { return; }
...
void cq3_op_3x4_whereBranchSync::notify_and_cancel(bool early) {
...
void cq3_op_3x4_whereBranchSync::initialize() {
...
void cq3_op_3x4_whereBranchSync::finalize() {
