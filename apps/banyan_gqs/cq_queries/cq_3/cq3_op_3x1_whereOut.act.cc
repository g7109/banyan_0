...
void cq3_op_3x1_whereOut::process(VertexBatch &&input) {
...
void cq3_op_3x1_whereOut::receive_eos(PathEos &&eos) {
    _ds_manager->receive_eos_by(std::move(eos));
}

void cq3_op_3x1_whereOut::initialize() {
...
void cq3_op_3x1_whereOut::finalize() {
    delete _ds_manager;
}
