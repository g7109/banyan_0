...
#include "cq6x_executor.hh"
#include "cq6x_op_3_globalSync_limit.act.h"

void cq6x_op_3_globalSync_limit::process(VertexBatch &&input) {
...
void cq6x_op_3_globalSync_limit::receive_eos(Eos&& eos) {
    if (_cancelled) { return; }
...
void cq6x_op_3_globalSync_limit::initialize() {
    _limit_num = cq_limit_n;
}

void cq6x_op_3_globalSync_limit::finalize() {
}

void cq6x_op_3_globalSync_limit::finish_query() {
    _cancelled = true;
    if (at_cq) {
        cq_end_action();
    }
...
void cq6x_op_3_globalSync_limit::log_results() {
    write_log("[CQ 6x finished]\n");
    for (auto& person : _friends) {
        write_log(seastar::format("  PersonId = {}\n", person));
    }
}

void cq6x_op_3_globalSync_limit::cq_end_action() {
