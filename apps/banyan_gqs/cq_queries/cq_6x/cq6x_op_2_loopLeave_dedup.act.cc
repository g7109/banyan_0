...
void cq6x_op_2_loopLeave_dedup::process(VertexBatch &&input) {
...
void cq6x_op_2_loopLeave_dedup::receive_eos(Eos &&eos) {
    _ds_sync_hdl->receive_eos();
}

void cq6x_op_2_loopLeave_dedup::initialize() {
    auto builder = get_scope();
    builder.reset_shard(0);
    _ds_sync_hdl = new ds_and_sync_t(builder, 3);
}

void cq6x_op_2_loopLeave_dedup::finalize() {
    delete _ds_sync_hdl;
}
