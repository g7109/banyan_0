...
#include "cq6_op_1x1x3_whereFilter.act.h"
#include "cq6_op_1x1x4_whereBranchSync.act.h"

void cq6_op_1x1x3_whereFilter::process(VertexBatch &&input) {
    if (_found) { return; }
    ...
    for (unsigned i = 0; i < input.size(); i++) {
        ...
        auto match = storage::berkeleydb_helper::compareString(
                std::get<0>(prop_loc_info), std::get<1>(prop_loc_info), _expect_type_name);
        if (match) {
...
void cq6_op_1x1x3_whereFilter::receive_eos(PathEos &&eos) {
    _sync_ref->receive_eos(std::move(eos));
}

void cq6_op_1x1x3_whereFilter::initialize() {
...
void cq6_op_1x1x3_whereFilter::finalize() {
    delete _sync_ref;
}
