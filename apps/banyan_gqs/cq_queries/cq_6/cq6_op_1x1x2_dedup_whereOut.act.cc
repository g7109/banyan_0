...
void cq6_op_1x1x2_dedup_whereOut::process(VertexBatch &&input) {
...
void cq6_op_1x1x2_dedup_whereOut::receive_eos(PathEos &&eos) {
    _ds_manager->receive_eos_by(std::move(eos));
}

void cq6_op_1x1x2_dedup_whereOut::initialize() {
...
void cq6_op_1x1x2_dedup_whereOut::finalize() {
    delete _ds_manager;
}
