//
// Created by everlighting on 5051/4/8.
//

#include "berkeley_db/storage/bdb_graph_handler.hh"
#include "common/query_helpers.hh"
#include "common/schedule_helpers.hh"
#include "cq5x_op_0_V_loopEnter.act.h"
#include "cq5x_op_1x0_loopOut.act.h"

void cq5x_op_0_V_loopEnter::trigger(StartVertex&& src) {
    _src_vertex = src.val;
    ...
    cq5_helper.set_organisation_ids(organisations);
...
seastar::future<TrivialStruct<bool> > cq5x_op_0_V_loopEnter::set_params(VertexBatch &&input) {
    cq5_helper.set_organisation_ids(std::move(input));
    return seastar::make_ready_future<TrivialStruct<bool> >(TrivialStruct<bool>{true});
}

void cq5x_op_0_V_loopEnter::forward() {
    _ds_group_hdl->emplace_data(brane::global_shard_id(), _src_vertex);
    _ds_group_hdl->flush();
    _ds_group_hdl->receive_eos();
}

void cq5x_op_0_V_loopEnter::initialize() {
    auto builder = get_scope();
...
void cq5x_op_0_V_loopEnter::finalize() {
