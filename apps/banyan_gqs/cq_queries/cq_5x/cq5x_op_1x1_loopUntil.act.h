...
class i_cq5x_op_2_loopLeave_dedup_in;

class ANNOTATION(actor:reference) i_cq5x_op_1x1_loopUntil : public brane::reference_base {
public:
    void process(VertexBatch&& input);
    void receive_eos(Eos&& eos);

    // Constructor.
    ACTOR_ITFC_CTOR(i_cq5x_op_1x1_loopUntil);
    // Destructor
    ACTOR_ITFC_DTOR(i_cq5x_op_1x1_loopUntil);
};

class ANNOTATION(actor:implement) cq5x_op_1x1_loopUntil : public brane::stateless_actor {
public:
    void process(VertexBatch&& input);
    void receive_eos(Eos&& eos);
    ...
    // Constructor.
    ACTOR_IMPL_CTOR(cq5x_op_1x1_loopUntil);
    // Destructor
    ACTOR_IMPL_DTOR(cq5x_op_1x1_loopUntil);
    // Do work
    ACTOR_DO_WORK();
...
private:
    ...
    using next_loop_t = downstream_handler<i_cq5x_op_1x0_loopOut, false, VertexBatch>;
    using loop_sync_t = downstream_handler<i_cq5x_op_2_loopLeave_dedup_in, true, VertexBatch>;
