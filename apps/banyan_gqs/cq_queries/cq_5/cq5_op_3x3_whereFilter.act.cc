...
void cq5_op_3x3_whereFilter::process(VertexBatch &&input) {
    if (_found) { return; }
...
void cq5_op_3x3_whereFilter::receive_eos(PathEos &&eos) {
    _sync_ref->receive_eos(std::move(eos));
}

void cq5_op_3x3_whereFilter::initialize() {
...
void cq5_op_3x3_whereFilter::finalize() {
    delete _sync_ref;
}
