...
#include "cq5_op_1x0_loopOut.act.h"
#include "cq5_op_2_loopLeave_dedup.act.h"

thread_local std::unordered_set<int64_t> cq5_op_1x1_loopUntil::_successful_vertices{};
thread_local std::unordered_set<int64_t> cq5_op_1x1_loopUntil::_failed_vertices{};

void cq5_op_1x1_loopUntil::process(VertexBatch &&input) {
...
void cq5_op_1x1_loopUntil::receive_eos(Eos &&eos) {
    _notify_ds |= eos.val;
...
void cq5_op_1x1_loopUntil::initialize() {
    ...
    for (auto& company : cq5_helper.get_organisation_ids()) {
        _expect_universities.insert(company);
    }
}

void cq5_op_1x1_loopUntil::finalize() {
