...
#include "cq5_op_3x1_whereOut.act.h"
#include "cq5_op_3x2_dedup_whereOut.act.h"
#include "cq5_op_3x4_whereBranchSync.act.h"

void cq5_op_3x1_whereOut::process(VertexBatch &&input) {
...
void cq5_op_3x1_whereOut::receive_eos(PathEos &&eos) {
    _ds_manager->receive_eos_by(std::move(eos));
}

void cq5_op_3x1_whereOut::initialize() {
...
void cq5_op_3x1_whereOut::finalize() {
    delete _ds_manager;
}
