...
inline void benchmark_exec_query_7(int64_t start_vertex) {
    write_log(seastar::format(
            "[Exec query 7] query id = {}, start vertex = {}\n",
            q7_helper.current_query_id(), start_vertex));
    brane::scope_builder builder(locate_helper::locate(start_vertex),
                                 brane::scope<brane::actor_group<> >(q7_helper.current_query_id()));
    auto actor_ref_V = builder.build_ref<i_q7_op_0_V>(0);

    q7_helper.record_start_time();
