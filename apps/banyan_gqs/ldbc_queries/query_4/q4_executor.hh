...
inline void benchmark_exec_query_4(int64_t start_vertex) {
    write_log(seastar::format(
            "[Exec query 4] query id = {}, start vertex = {}\n",
            q4_helper.current_query_id(), start_vertex));
    brane::scope_builder builder(locate_helper::locate(start_vertex),
                                 brane::scope<brane::actor_group<> >(q4_helper.current_query_id()));
    auto actor_ref_V = builder.build_ref<i_q4_op_0_V>(0);

    q4_helper.record_start_time();
