...
arg_parser.add_argument('--libclang-path', action='store', dest='libclang_path',
                        help='Libclang path on your system')
arg_parser.add_argument('--open-gpu-resource-pool', action='store_true', dest='open_gpu_resource_pool',
                        help='Open gpu resource pool in brane')
add_tristate(
    arg_parser,
    name = 'dpdk',
    dest = 'dpdk',
...
def configure_mode(mode):
    ...
    TRANSLATED_ARGS = [
        ...
        '-DLIBCLANG_PATH={}'.format(args.libclang_path),
        '-DBRANE_GPU_RESOURCE_POOL={}'.format(args.open_gpu_resource_pool),
