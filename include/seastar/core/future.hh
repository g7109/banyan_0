...
public:
    ...
    explicit continuation_base(brane::actor_base* exec_ctx = nullptr) : task(exec_ctx) {}
    explicit continuation_base(future_state<T...>&& state, brane::actor_base* exec_ctx = nullptr)
        : task(exec_ctx), _state(std::move(state)) {}
...
struct continuation final : continuation_base<T...> {
    // [DAIL] Modify constructors to adapt with execution context.
    continuation(Func&& func, future_state<T...>&& state, brane::actor_base* exec_ctx = nullptr)
        : continuation_base<T...>(std::move(state), exec_ctx), _func(std::move(func)) {}
    continuation(Func&& func, brane::actor_base* exec_ctx = nullptr)
        : continuation_base<T...>(exec_ctx), _func(std::move(func)) {}
    ...
    void cancel() override {
        if (!this->_state.failed()) {
            this->_state = {};
            this->_state.set_exception(std::make_exception_ptr(std::runtime_error("External cancel")));
        }
        // fmt::print("[INFO] in continuation cancel(), _state type is {} after setting\n", (int)(this->_state._u.st));
    }
    Func _func;
};
...
private:
    ...
    void schedule(Func&& func, brane::actor_base* exec_ctx = nullptr) {
        auto tws = std::make_unique<continuation<Func, T...>>(std::move(func), exec_ctx);
        _state = &tws->_state;
        _task = std::move(tws);
    }
...
private:
    ...
    void schedule(Func&& func, brane::actor_base* exec_ctx = nullptr) {
        if (_state.available() || !_promise) {
            ...
            ::seastar::schedule(std::make_unique<continuation<Func, T...>>(std::move(func), std::move(_state), exec_ctx));
        } else {
            assert(_promise);
            detach_promise()->schedule(std::move(func), exec_ctx);
        }
    }
...
public:
    ...
    Result
    then(Func&& func, brane::actor_base* exec_ctx = nullptr) noexcept {
#ifndef SEASTAR_TYPE_ERASE_MORE
        return then_impl(std::move(func), exec_ctx);
#else
...
private:
    // TODO[DAIL]: add execution_context
    template <typename Func, typename Result = futurize_t<std::result_of_t<Func(T&&...)>>>
    Result
    then_impl(Func&& func, brane::actor_base* exec_ctx = nullptr) noexcept {
        using futurator = futurize<std::result_of_t<Func(T&&...)>>;
...
public:
    ...
    Result
    then_wrapped(Func&& func, brane::actor_base* exec_ctx = nullptr) noexcept {
#ifndef SEASTAR_TYPE_ERASE_MORE
        return then_wrapped_impl(std::move(func), exec_ctx);
#else
...
private:
    ...
    template <typename Func, typename Result = futurize_t<std::result_of_t<Func(future)>>>
    Result
    then_wrapped_impl(Func&& func, brane::actor_base* exec_ctx = nullptr) noexcept {
        using futurator = futurize<std::result_of_t<Func(future)>>;
        if (available() && !need_preempt()) {
